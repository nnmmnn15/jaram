...
var q = {
    ...
    7: {"title": "한 번 참여해본 축제일 때 나는?", "type": "EH", "A": "한번 참여 해봤던 축제이니만큼 익숙한 루트로 빠르게 다니기?", "B": "새로운 부스도 많이 생겼네? 한번 가볼까?"} ,
    8: {"title": "오랜만에 보는 친구와 약속이 생겼다 (축제 전날?)", "type": "EH", "A": "이미 전날에 뭐 입을지 다 준비해놨지~~", "B": "이따 날씨보고~ 나갈 준비 하면서 생각하지 뭐~"} ,
    9: {"title": "가려고 했던 축제가 예상외로 규모가 작다.마침 근처에 다른 축제가 열린다 ", "type": "EH", "A": "계획은 지켜야지~ 조금 실망했지만 계획대로 움직인다", "B": "저기가 더 재밌어보인다! 라고 하며 다른 축제로 간다"} ,
}
...
function next() {
    if (num == 10) {
        ...
        ($("#EH").val() < 2) ? mbti += "E" : mbti += "H";

        window.location.href = "./jaram/result/" + mbti
    } else {
    $(".q-num").html('Q' + num);
